...
function footer()
{
    echo "<footer class=\"main-footer\">
        ...
        "</a>.</strong> SIM PKK Kampung Petta.";
...
function login($user, $pass)
{
    ...
    if (get_koneksi()) {
        if (!empty($user) && !empty($pass)) {
            ...
            $sql = "SELECT * FROM tb_admin WHERE username='$username' AND password=sha1('$password')";
...
function set_slug_news($berita, $pjg)
{
    $dt = explode(" ", $berita);
    for ($i = 0; $i < $pjg; $i++) {
        echo ucfirst($dt[$i]);
...
function frontend_view_news($tabel, $index, $field = array())
{
    if (get_koneksi()) {
        $sql = "select * from " . $tabel . " order by " . $index . " DESC ";
        $query = mysqli_query(get_koneksi(), $sql);
        ...
        while ($result = mysqli_fetch_assoc($query)) {
            ...
            echo "<ul class='my-3'>";
            echo "<li>";
            echo "<h5><i class='fa fa-calendar-o'></i> 10 Jun 2021</h5>";
            echo "</li>";
            echo "<li class='separator mx-2'><span></span></li>";
            echo "<li>";
            echo "<h5><i class='fa fa-folder-open-o'></i>Pokja PKK</h5>";
            echo "</li>";
            echo "</ul>";
            echo "</div>";
            echo "<div class='news-title'>";
            echo "<h4><a href='detailberita.php?id=". $result[$field[0]] ."'>";
            echo $result[$field[1]];
...
function frontend_view_galeri($tabel, $index, $field = array())
{
    if (get_koneksi()) {
        $sql = "select * from " . $tabel . " order by " . $index . " DESC ";
        $query = mysqli_query(get_koneksi(), $sql);
...
function frontend_view_pengurus($tabel, $index, $field = array())
{
    if (get_koneksi()) {
        $sql = "select * from " . $tabel . " order by " . $index . " DESC ";
        $query = mysqli_query(get_koneksi(), $sql);
